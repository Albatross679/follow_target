...
# ===========================================================================
# Simplified configs — only the knobs a user needs to touch
# ===========================================================================
#
# Usage:
#     config = SimpleSAC(num_nodes=11, total_frames=5_000_000, learning_rate=1e-3)
#     config = SimplePPO(num_nodes=21, total_frames=10_000_000, learning_rate=3e-4)
#     config = SimpleMMRKHS(num_nodes=21, total_frames=5_000_000)
#
# Everything else inherits paper defaults from Choi2025*Config.
# ===========================================================================
...
class SimpleSAC(Choi2025Config):
    """Simplified SAC config — only the essentials.

    Inherits all paper defaults. Override what you need:
    ...
    # --- Tunable knobs ---
    num_nodes: int = 21
    learning_rate: float = 0.001  # Sets both actor_lr and critic_lr
    total_frames: int = 20_000_000
    batch_size: int = 2048
    num_envs: int = 500
    max_wall_time: Optional[float] = None
...
class SimplePPO(Choi2025PPOConfig):
    """Simplified PPO config — only the essentials.
...
class SimpleMMRKHS(Choi2025MMRKHSConfig):
    """Simplified MM-RKHS config — only the essentials.

    Inherits all paper defaults (including notebook mechanics).
